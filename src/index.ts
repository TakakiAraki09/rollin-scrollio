...
type RollingScrollioOptions = RollingScrollioCallback &
	(RollingScrollioSelector | RollingScrollioElement | {});

function createScrollioInstances(options: RollingScrollioOptions = {}) {
	const contents = (() => {
		if ("selector" in options) {
			return getBySelector(options.selector).map((element) =>
				createScrollioInstance(element, {
		...
		return getBySelector(DEFAULT_WRAPPER_SELECTOR).map((element) =>
			createScrollioInstance(element, {
				callback: options.callback,
			}),
		);
